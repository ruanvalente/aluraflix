...
public class VideoResourcesService {
    ...
    public VideoResponseDTO findById(Long id) {
        if (Objects.isNull(id)){
            throw new VideoException(HttpStatus.PRECONDITION_FAILED, "Não foi possível realizar a operação. O ID é obrigatório");
        }
        Video video = service.findById(id);
        if (Objects.isNull(video)){
            throw new VideoException(HttpStatus.NOT_FOUND, "Não foi possível recuperar o recurso informado. Por favor contate o administrador do sistema.");
    ...
    public void delete(Long id) {
        ...
        if (Objects.isNull(id)) {
            throw new VideoException(HttpStatus.PRECONDITION_FAILED, "Não foi possível exluir o vídeo informado. O campo ID não" +
                    " pode ser nulo.");
        }
        Video videoRecovered = service.findById(id);
        if (Objects.isNull(videoRecovered)) {
